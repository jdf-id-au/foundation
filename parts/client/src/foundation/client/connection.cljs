...
(defn post!
  ...
  ;; TODO 2026-08-19 21:13:11 ,,, & {:keys [on-success on-failure]} when available
  ...
  (-> (js/fetch (if (vector? endpoint) (apply config/api endpoint) (config/api endpoint))
    ...
    (.then
      (fn [response]
        (let [status (oget response "status")]
          (case status
            200 (-> (.text response)
                  (.then
                    (fn [v] (-> v fm/decode fm/receive))
                    (fn [e] (log/warn "Failed to read" response e))))
            401 (log/warn "Unauthenticated" response)
            403 (log/warn "Unauthorised" response)
            (log/warn "Unsupported status" response))
          (when on-success (dispatch on-success {:response response :status status})))) ; "dispatch data will be merged into the original dispatch data"
